...
ctc_beam_search_decoder_batch_hotwords(
    ...
    const std::vector<std::string> &hot_words,
    const std::vector<double> &hot_words_weights, 
    int blank_id, int space_id, double cutoff_prob,
    Scorer *ext_scorer) {
  // thread pool
  ThreadPool pool(num_processes);
  // number of samples
  ...
  for (size_t i = 0; i < batch_size; ++i) {
    res.emplace_back(
        pool.enqueue(ctc_beam_search_decoder_hotwords, std::ref(batch_log_probs_seq[i]),
                     std::ref(batch_log_probs_idx[i]),
                     std::ref(*batch_root_trie[i]), batch_start[i], beam_size,
                     std::ref(hot_words), std::ref(hot_words_weights),blank_id, space_id, cutoff_prob, ext_scorer));
  }
